...
const SecurityCard = ({ apmtInfo, today, handleEdit }) => {
    return (
            ...
            >
                {apmtInfo?.vehicles?.map((vehicle, idx) => (
                    <View className={`at-row at-row__align--center ${idx%2 == 0 ? 'even' : 'odd'}`}>
                        <View className='at-col at-col-9 infoListing'>{vehicle.plate}</View>
                        <View className='at-col at-col__offset-1 infoListing info'><AtButton circle size='small' type='primary'>B</AtButton></View>
                    </View>
                ))}
            </AtCard>
        </View>
